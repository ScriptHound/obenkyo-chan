...
from src.queries.queries import KatakanaCard
...
@bl.on.message(text=["Катакана"])
async def greeting(message: Message):
    poll: dict = create_letters_poll(KatakanaCard, Session, engine)
    right_answer_index: int = random.choice(list(poll.keys()))
    right_answer: KatakanaCard = poll[right_answer_index].transcription
